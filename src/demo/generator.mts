...
export function DemoEntityGenerator({ scheduler, synapse, context, logger }: TServiceParams) {
  try {
    ...
    scheduler.setInterval(() => {
      ...
      // eslint-disable-next-line sonarjs/pseudo-random
      if (Math.random() > 0.8) {
        motionSensor.storage
          .set("is_on", !motionSensor.storage.get("is_on"))
          .catch((error: unknown) => {
            logger.error("Failed to update motion sensor", error);
          });
      }
    ...
    demoButton.onPress(() => {
      ...
      const currentState = demoSwitch.storage.get("is_on") || false;
      demoSwitch.storage.set("is_on", !currentState).catch((error: unknown) => {
        logger.error("Failed to toggle demo switch", error);
      });
    });
