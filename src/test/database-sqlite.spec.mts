...
import { sqliteHomeAssistantEntity } from "../schema/sqlite.mts";
import { sqliteHomeAssistantEntityLocals } from "../schema/sqlite.mts";
...
it("exists", async () => {
  ...
  await testRunner().run(({ synapse }) => {
    expect(synapse.db_sqlite).toBeDefined();
  });
});
...
describe("update", () => {
  it("should insert new entity when it doesn't exist", async () => {
    ...
    await testRunner().run(async ({ synapse, config, internal }) => {
      ...
      await synapse.db_sqlite.update(unique_id, content, defaults);

      // Query database directly to verify insertion
      const database = synapse.db_sqlite.getDatabase() as BetterSQLite3Database;
  ...
  it("should update existing entity when it already exists", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      await synapse.db_sqlite.update(unique_id, initialContent, defaults);

      // Update the entity
      await synapse.db_sqlite.update(unique_id, updatedContent, defaults);
  ...
  it("should handle onConflictDoUpdate correctly", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      // First insert
      await synapse.db_sqlite.update(unique_id, content1, defaults);

      // Second insert with same unique_id (should trigger conflict resolution)
      await synapse.db_sqlite.update(unique_id, content2, defaults);
  ...
  it("should handle errors and re-throw them", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      await expect(synapse.db_sqlite.update(unique_id, content, defaults)).rejects.toThrow(
...
describe("load", () => {
  it("should create new entity when no rows are found during load", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      const result = await synapse.db_sqlite.load(unique_id, defaults);
  ...
  it("should use registered defaults when defaults parameter is not provided", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      await synapse.db_sqlite.load(unique_id, registeredDefaults);

      // Second call without defaults (should use registered defaults)
      await synapse.db_sqlite.update(unique_id, content);
  ...
  it("should return early when defaults are equal to current base state", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      const firstResult = await synapse.db_sqlite.load(unique_id, defaults);
      ...
      const secondResult = await synapse.db_sqlite.load(unique_id, defaults);
  ...
  it("should re-throw unexpected errors", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      await expect(synapse.db_sqlite.load(unique_id, defaults)).rejects.toThrow(
        "Database connection failed",
      );
  ...
  it("should return existing entity without reset when REBUILD_ON_ENTITY_CHANGE is false and defaults differ", async () => {
    ...
    await testRunner()
      ...
      .run(async ({ synapse, config }) => {
        ...
        const firstResult = await synapse.db_sqlite.load(unique_id, initialDefaults);
        ...
        const secondResult = await synapse.db_sqlite.load(unique_id, newDefaults);
...
describe("locals", () => {
  it("should insert new local when it doesn't exist", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      await synapse.db_sqlite.updateLocal(unique_id, key, content);
  ...
  it("should update existing local when it already exists", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      await synapse.db_sqlite.updateLocal(unique_id, key, initialContent);

      // Update the local
      await synapse.db_sqlite.updateLocal(unique_id, key, updatedContent);
  ...
  it("should delete local when content is undefined", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      await synapse.db_sqlite.updateLocal(unique_id, key, undefined);
  ...
  it("should load all locals for a unique_id", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      for (const local of locals) {
        await synapse.db_sqlite.updateLocal(unique_id, local.key, local.content);
      }

      // Load all locals
      const result = await synapse.db_sqlite.loadLocals(unique_id);
  ...
  it("should delete specific local by key", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      // Insert two locals
      await synapse.db_sqlite.updateLocal(unique_id, key1, content);
      await synapse.db_sqlite.updateLocal(unique_id, key2, content);

      // Delete one local
      await synapse.db_sqlite.deleteLocal(unique_id, key1);
  ...
  it("should delete all locals for a unique_id", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      // Delete all locals
      await synapse.db_sqlite.deleteLocalsByUniqueId(unique_id);
  ...
  it("should handle errors and re-throw them", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      await expect(synapse.db_sqlite.updateLocal(unique_id, key, content)).rejects.toThrow(
        "Database connection failed",
      );
...
describe("deleteEntity", () => {
  it("should delete entity when it exists", async () => {
    ...
    await testRunner().run(async ({ synapse, config }) => {
      ...
      // Delete the entity
      await synapse.db_sqlite.deleteEntity(unique_id);
  ...
  it("should handle deleting non-existent entity gracefully", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      await expect(synapse.db_sqlite.deleteEntity(unique_id)).resolves.not.toThrow();
    });
  });
  ...
  it("should handle errors and re-throw them", async () => {
    ...
    await testRunner().run(async ({ synapse }) => {
      ...
      await expect(synapse.db_sqlite.deleteEntity(unique_id)).rejects.toThrow(
        "Database connection failed",
      );
